---
title: Pack200.Packer.pack()
permalink: Java/Pack200/Packer/pack
date: 2021-01-04
key: JavaJava.P.Pack200.Packer
category: java
tags: ['java se', 'java.util.jar', 'java.base', 'metodo java', 'Java 1.5']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.P.Pack200.Packer.metodos valor="pack" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
void pack(JarFile in, OutputStream out) throws IOException
void pack(JarInputStream in, OutputStream out) throws IOException
~~~

## Parámetros
* **JarFile in**,  {% include w3api/param_description.html metodo=_data parametro="JarFile in" %}
* **JarInputStream in**,  {% include w3api/param_description.html metodo=_data parametro="JarInputStream in" %}
* **OutputStream out**,  {% include w3api/param_description.html metodo=_data parametro="OutputStream out" %}

## Excepciones
[IOException](/Java/IOException/)

## Clase Padre
[Pack200.Packer](/Java/Pack200/Packer/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.P.Pack200.Packer.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
