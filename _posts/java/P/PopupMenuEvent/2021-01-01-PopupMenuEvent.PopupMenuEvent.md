---
title: PopupMenuEvent.PopupMenuEvent()
permalink: Java/PopupMenuEvent/PopupMenuEvent
date: 2021-01-11
key: JavaJava.P.PopupMenuEvent
category: java
tags: ['java se', 'javax.swing.event', 'java.desktop', 'metodo java', 'Java 1.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.P.PopupMenuEvent.constructores valor="PopupMenuEvent" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public PopupMenuEvent(Object source)
~~~

## Parámetros
* **Object source**,  {% include w3api/param_description.html metodo=_dato parametro="Object source" %}

## Clase Padre
[PopupMenuEvent](/Java/PopupMenuEvent/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in _dato.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
