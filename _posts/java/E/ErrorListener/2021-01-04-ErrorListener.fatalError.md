---
title: ErrorListener.fatalError()
permalink: Java/ErrorListener/fatalError
date: 2021-01-04
key: JavaJava.E.ErrorListener
category: java
tags: ['java se', 'javax.xml.transform', 'java.xml', 'metodo java', 'Java 1.4']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.E.ErrorListener.metodos valor="fatalError" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
void fatalError(TransformerException exception) throws TransformerException
~~~

## Parámetros
* **TransformerException exception**,  {% include w3api/param_description.html metodo=_data parametro="TransformerException exception" %}

## Excepciones
[TransformerException](/Java/TransformerException/)

## Clase Padre
[ErrorListener](/Java/ErrorListener/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.E.ErrorListener.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
