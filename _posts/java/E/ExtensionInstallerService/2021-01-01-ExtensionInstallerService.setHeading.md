---
title: ExtensionInstallerService.setHeading()
permalink: Java/ExtensionInstallerService/setHeading
date: 2021-01-11
key: JavaJava.E.ExtensionInstallerService
category: java
tags: ['java se', 'javax.jnlp', 'java.jnlp', 'metodo java', 'Java 1.4.2']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.E.ExtensionInstallerService.metodos valor="setHeading" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
void setHeading(String heading)
~~~

## Parámetros
* **String heading**,  {% include w3api/param_description.html metodo=_dato parametro="String heading" %}

## Clase Padre
[ExtensionInstallerService](/Java/ExtensionInstallerService/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in _dato.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
