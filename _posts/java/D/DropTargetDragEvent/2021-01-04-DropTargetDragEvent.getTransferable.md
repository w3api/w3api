---
title: DropTargetDragEvent.getTransferable()
permalink: Java/DropTargetDragEvent/getTransferable
date: 2021-01-04
key: JavaJava.D.DropTargetDragEvent
category: java
tags: ['java se', 'java.awt.dnd', 'java.desktop', 'metodo java', 'Java 1.2']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.D.DropTargetDragEvent.metodos valor="getTransferable" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public Transferable getTransferable()
~~~

## Excepciones
[InvalidDnDOperationException](/Java/InvalidDnDOperationException/)

## Clase Padre
[DropTargetDragEvent](/Java/DropTargetDragEvent/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.D.DropTargetDragEvent.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
