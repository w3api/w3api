---
title: Document.adoptNode()
permalink: Java/Document-org-w3c-dom/adoptNode
date: 2021-01-04
key: JavaJava.D.Document-org-w3c-dom
category: java
tags: ['java se', 'org.w3c.dom', 'java.xml', 'metodo java', 'Java 1.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.D.Document-org-w3c-dom.metodos valor="adoptNode" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
Node adoptNode(Node source) throws DOMException
~~~

## Parámetros
* **Node source**,  {% include w3api/param_description.html metodo=_data parametro="Node source" %}

## Clase Padre
[Document](/Java/Document-org-w3c-dom/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.D.Document-org-w3c-dom.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
