---
title: NamingManager.setInitialContextFactoryBuilder()
permalink: Java/NamingManager/setInitialContextFactoryBuilder
date: 2021-01-04
key: JavaJava.N.NamingManager
category: java
tags: ['java se', 'javax.naming.spi', 'java.naming', 'metodo java', 'Java 1.3']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.N.NamingManager.metodos valor="setInitialContextFactoryBuilder" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public static void setInitialContextFactoryBuilder(InitialContextFactoryBuilder builder) throws NamingException
~~~

## Parámetros
* **InitialContextFactoryBuilder builder**,  {% include w3api/param_description.html metodo=_data parametro="InitialContextFactoryBuilder builder" %}

## Excepciones
[NamingException](/Java/NamingException/), [SecurityException](/Java/SecurityException/), [IllegalStateException](/Java/IllegalStateException/)

## Clase Padre
[NamingManager](/Java/NamingManager/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.N.NamingManager.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
