---
title: NoContextHelper.write()
permalink: Java/NoContextHelper/write
date: 2021-01-11
key: JavaJava.N.NoContextHelper
category: java
tags: ['java se', 'org.omg.PortableServer.CurrentPackage', 'java.corba', 'metodo java', 'Java 1.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.N.NoContextHelper.metodos valor="write" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public static void write(OutputStream ostream, NoContext value)
~~~

## Parámetros
* **NoContext value**,  {% include w3api/param_description.html metodo=_dato parametro="NoContext value" %}
* **OutputStream ostream**,  {% include w3api/param_description.html metodo=_dato parametro="OutputStream ostream" %}

## Clase Padre
[NoContextHelper](/Java/NoContextHelper/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in _dato.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
