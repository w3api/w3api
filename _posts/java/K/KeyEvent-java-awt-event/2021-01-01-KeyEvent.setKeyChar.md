---
title: KeyEvent.setKeyChar()
permalink: Java/KeyEvent-java-awt-event/setKeyChar
date: 2021-01-11
key: JavaJava.K.KeyEvent-java-awt-event
category: java
tags: ['java se', 'java.awt.event', 'java.desktop', 'metodo java', 'Java 1.1']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.K.KeyEvent-java-awt-event.metodos valor="setKeyChar" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public void setKeyChar(char keyChar)
~~~

## Parámetros
* **char keyChar**,  {% include w3api/param_description.html metodo=_dato parametro="char keyChar" %}

## Clase Padre
[KeyEvent](/Java/KeyEvent-java-awt-event/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in _dato.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
