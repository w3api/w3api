---
title: ColorSupported.ColorSupported()
permalink: Java/ColorSupported/ColorSupported
date: 2021-01-11
key: JavaJava.C.ColorSupported
category: java
tags: ['java se', 'javax.print.attribute.standard', 'java.desktop', 'metodo java', 'Java 1.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.C.ColorSupported.constructores valor="ColorSupported" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
protected ColorSupported(int value)
~~~

## Parámetros
* **int value**,  {% include w3api/param_description.html metodo=_dato parametro="int value" %}

## Clase Padre
[ColorSupported](/Java/ColorSupported/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in _dato.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
