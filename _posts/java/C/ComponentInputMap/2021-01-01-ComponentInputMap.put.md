---
title: ComponentInputMap.put()
permalink: Java/ComponentInputMap/put
date: 2021-01-11
key: JavaJava.C.ComponentInputMap
category: java
tags: ['java se', 'javax.swing', 'java.desktop', 'metodo java', 'Java 1.3']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.C.ComponentInputMap.metodos valor="put" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public void put(KeyStroke keyStroke, Object actionMapKey)
~~~

## Parámetros
* **KeyStroke keyStroke**,  {% include w3api/param_description.html metodo=_dato parametro="KeyStroke keyStroke" %}
* **Object actionMapKey**,  {% include w3api/param_description.html metodo=_dato parametro="Object actionMapKey" %}

## Clase Padre
[ComponentInputMap](/Java/ComponentInputMap/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in _dato.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
