---
title: ChannelBinding.ChannelBinding()
permalink: Java/ChannelBinding/ChannelBinding
date: 2021-01-04
key: JavaJava.C.ChannelBinding
category: java
tags: ['java se', 'org.ietf.jgss', 'java.security.jgss', 'metodo java', 'Java 1.4']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.C.ChannelBinding.constructores valor="ChannelBinding" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public ChannelBinding(byte[] appData)
public ChannelBinding(InetAddress initAddr, InetAddress acceptAddr, byte[] appData)
~~~

## Parámetros
* **InetAddress initAddr**,  {% include w3api/param_description.html metodo=_data parametro="InetAddress initAddr" %}
* **byte[] appData**,  {% include w3api/param_description.html metodo=_data parametro="byte[] appData" %}
* **InetAddress acceptAddr**,  {% include w3api/param_description.html metodo=_data parametro="InetAddress acceptAddr" %}

## Clase Padre
[ChannelBinding](/Java/ChannelBinding/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.C.ChannelBinding.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
