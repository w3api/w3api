---
title: Connection.TRANSACTION_REPEATABLE_READ
permalink: Java/Connection-java-sql/TRANSACTION_REPEATABLE_READ
date: 2021-01-04
key: JavaJava.C.Connection-java-sql
category: java
tags: ['java se', 'java.sql', 'java.sql', 'campo java', 'Java 1.1']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.C.Connection-java-sql.campos valor="TRANSACTION_REPEATABLE_READ" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
static int TRANSACTION_REPEATABLE_READ
~~~

## Clase Padre
[Connection](/Java/Connection-java-sql/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.C.Connection-java-sql.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
