---
title: ImageIO.read()
permalink: Java/ImageIO/read
date: 2021-01-04
key: JavaJava.I.ImageIO
category: java
tags: ['java se', 'javax.imageio', 'java.desktop', 'metodo java', 'Java 1.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.I.ImageIO.metodos valor="read" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public static BufferedImage read(File input) throws IOException
public static BufferedImage read(InputStream input) throws IOException
public static BufferedImage read(URL input) throws IOException
public static BufferedImage read(ImageInputStream stream) throws IOException
~~~

## Parámetros
* **InputStream input**,  {% include w3api/param_description.html metodo=_data parametro="InputStream input" %}
* **URL input**,  {% include w3api/param_description.html metodo=_data parametro="URL input" %}
* **ImageInputStream stream**,  {% include w3api/param_description.html metodo=_data parametro="ImageInputStream stream" %}
* **File input**,  {% include w3api/param_description.html metodo=_data parametro="File input" %}

## Excepciones
[IOException](/Java/IOException/), [IllegalArgumentException](/Java/IllegalArgumentException/)

## Clase Padre
[ImageIO](/Java/ImageIO/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.I.ImageIO.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
