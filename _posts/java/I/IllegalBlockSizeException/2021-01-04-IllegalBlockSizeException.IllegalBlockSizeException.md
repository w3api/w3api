---
title: IllegalBlockSizeException.IllegalBlockSizeException()
permalink: Java/IllegalBlockSizeException/IllegalBlockSizeException
date: 2021-01-04
key: JavaJava.I.IllegalBlockSizeException
category: java
tags: ['java se', 'javax.crypto', 'java.base', 'metodo java', 'Java 1.4']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.I.IllegalBlockSizeException.constructores valor="IllegalBlockSizeException" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public IllegalBlockSizeException()
public IllegalBlockSizeException(String msg)
~~~

## Parámetros
* **String msg**,  {% include w3api/param_description.html metodo=_data parametro="String msg" %}

## Clase Padre
[IllegalBlockSizeException](/Java/IllegalBlockSizeException/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.I.IllegalBlockSizeException.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
