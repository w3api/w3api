---
title: FlatteningPathIterator.next()
permalink: Java/FlatteningPathIterator/next
date: 2021-01-04
key: JavaJava.F.FlatteningPathIterator
category: java
tags: ['java se', 'java.awt.geom', 'java.desktop', 'metodo java', 'Java 1.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.F.FlatteningPathIterator.metodos valor="next" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public void next()
~~~

## Clase Padre
[FlatteningPathIterator](/Java/FlatteningPathIterator/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.F.FlatteningPathIterator.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
