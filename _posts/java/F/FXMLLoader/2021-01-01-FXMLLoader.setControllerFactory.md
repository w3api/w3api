---
title: FXMLLoader.setControllerFactory()
permalink: Java/FXMLLoader/setControllerFactory
date: 2021-01-11
key: JavaJava.F.FXMLLoader
category: java
tags: ['java se', 'javafx.fxml', 'javafx.fxml', 'metodo java', 'JavaFX 2.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.F.FXMLLoader.metodos valor="setControllerFactory" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public void setControllerFactory(Callback<Class<?>,Object> controllerFactory)
~~~

## Parámetros
* **Object&gt; controllerFactory**,  {% include w3api/param_description.html metodo=_dato parametro="Object> controllerFactory" %}
* **Callback&lt;Class&lt;?&gt;**,  {% include w3api/param_description.html metodo=_dato parametro="Callback<Class<?>" %}

## Clase Padre
[FXMLLoader](/Java/FXMLLoader/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in _dato.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
