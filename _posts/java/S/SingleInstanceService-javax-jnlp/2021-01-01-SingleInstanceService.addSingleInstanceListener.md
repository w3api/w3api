---
title: SingleInstanceService.addSingleInstanceListener()
permalink: /Java/SingleInstanceService-javax-jnlp/addSingleInstanceListener/
date: 2021-01-11
key: Java.S.SingleInstanceService-javax-jnlp
category: Java
tags: ['java se', 'javax.jnlp', 'java.jnlp', 'metodo java', 'Java 1.5']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.S.SingleInstanceService-javax-jnlp.metodos valor="addSingleInstanceListener" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
void addSingleInstanceListener(SingleInstanceListener sil)
~~~

## Parámetros
* **SingleInstanceListener sil**,  {% include w3api/param_description.html metodo=_dato parametro="SingleInstanceListener sil" %}

## Clase Padre
[SingleInstanceService](/Java/SingleInstanceService-javax-jnlp/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in _dato.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
