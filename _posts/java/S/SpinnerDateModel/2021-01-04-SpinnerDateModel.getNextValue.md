---
title: SpinnerDateModel.getNextValue()
permalink: Java/SpinnerDateModel/getNextValue
date: 2021-01-04
key: JavaJava.S.SpinnerDateModel
category: java
tags: ['java se', 'javax.swing', 'java.desktop', 'metodo java', 'Java 1.4']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.S.SpinnerDateModel.metodos valor="getNextValue" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public Object getNextValue()
~~~

## Clase Padre
[SpinnerDateModel](/Java/SpinnerDateModel/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.S.SpinnerDateModel.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
