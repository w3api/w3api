---
title: X509Certificate.getExtendedKeyUsage()
permalink: Java/X509Certificate-java-security-cert/getExtendedKeyUsage
date: 2021-01-04
key: JavaJava.X.X509Certificate-java-security-cert
category: java
tags: ['java se', 'java.security.cert', 'java.base', 'metodo java', 'Java 1.2']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.X.X509Certificate-java-security-cert.metodos valor="getExtendedKeyUsage" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public List<String> getExtendedKeyUsage() throws CertificateParsingException
~~~

## Excepciones
[CertificateParsingException](/Java/CertificateParsingException/)

## Clase Padre
[X509Certificate](/Java/X509Certificate-java-security-cert/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.X.X509Certificate-java-security-cert.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
