---
title: Transform.shear()
permalink: /Java/Transform-javafx-scene-transform/shear/
date: 2021-01-11
key: Java.T.Transform-javafx-scene-transform
category: Java
tags: ['java se', 'javafx.scene.transform', 'javafx.graphics', 'metodo java', 'JavaFX 2.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.T.Transform-javafx-scene-transform.metodos valor="shear" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public static Shear shear(double x, double y)
public static Shear shear(double x, double y, double pivotX, double pivotY)
~~~

## Parámetros
* **double x**,  {% include w3api/param_description.html metodo=_dato parametro="double x" %}
* **double pivotY**,  {% include w3api/param_description.html metodo=_dato parametro="double pivotY" %}
* **double y**,  {% include w3api/param_description.html metodo=_dato parametro="double y" %}
* **double pivotX**,  {% include w3api/param_description.html metodo=_dato parametro="double pivotX" %}

## Clase Padre
[Transform](/Java/Transform-javafx-scene-transform/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in _dato.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
