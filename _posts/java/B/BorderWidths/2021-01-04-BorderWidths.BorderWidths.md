---
title: BorderWidths.BorderWidths()
permalink: Java/BorderWidths/BorderWidths
date: 2021-01-04
key: JavaJava.B.BorderWidths
category: java
tags: ['java se', 'javafx.scene.layout', 'javafx.graphics', 'metodo java', 'JavaFX 8.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.B.BorderWidths.constructores valor="BorderWidths" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public BorderWidths(double width)
public BorderWidths(double top, double right, double bottom, double left)
public BorderWidths(double top, double right, double bottom, double left, boolean topAsPercentage, boolean rightAsPercentage, boolean bottomAsPercentage, boolean leftAsPercentage)
~~~

## Parámetros
* **double top**,  {% include w3api/param_description.html metodo=_data parametro="double top" %}
* **double width**,  {% include w3api/param_description.html metodo=_data parametro="double width" %}
* **double left**,  {% include w3api/param_description.html metodo=_data parametro="double left" %}
* **boolean topAsPercentage**,  {% include w3api/param_description.html metodo=_data parametro="boolean topAsPercentage" %}
* **boolean leftAsPercentage**,  {% include w3api/param_description.html metodo=_data parametro="boolean leftAsPercentage" %}
* **boolean bottomAsPercentage**,  {% include w3api/param_description.html metodo=_data parametro="boolean bottomAsPercentage" %}
* **double bottom**,  {% include w3api/param_description.html metodo=_data parametro="double bottom" %}
* **boolean rightAsPercentage**,  {% include w3api/param_description.html metodo=_data parametro="boolean rightAsPercentage" %}
* **double right**,  {% include w3api/param_description.html metodo=_data parametro="double right" %}

## Clase Padre
[BorderWidths](/Java/BorderWidths/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.B.BorderWidths.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
