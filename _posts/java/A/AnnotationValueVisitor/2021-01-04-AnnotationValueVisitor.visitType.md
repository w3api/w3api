---
title: AnnotationValueVisitor.visitType()
permalink: Java/AnnotationValueVisitor/visitType
date: 2021-01-04
key: JavaJava.A.AnnotationValueVisitor
category: java
tags: ['java se', 'javax.lang.model.element', 'java.compiler', 'metodo java', 'Java 1.6']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.A.AnnotationValueVisitor.metodos valor="visitType" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
R visitType(TypeMirror t, P p)
~~~

## Parámetros
* **TypeMirror t**,  {% include w3api/param_description.html metodo=_data parametro="TypeMirror t" %}
* **P p**,  {% include w3api/param_description.html metodo=_data parametro="P p" %}

## Clase Padre
[AnnotationValueVisitor](/Java/AnnotationValueVisitor/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.A.AnnotationValueVisitor.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
