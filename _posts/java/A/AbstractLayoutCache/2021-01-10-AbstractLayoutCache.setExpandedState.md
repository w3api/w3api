---
title: AbstractLayoutCache.setExpandedState()
permalink: Java/AbstractLayoutCache/setExpandedState
date: 2021-01-10
key: JavaJava.A.AbstractLayoutCache
category: java
tags: ['java se', 'javax.swing.tree', 'java.desktop', 'metodo java', 'Java 1.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.A.AbstractLayoutCache.metodos valor="setExpandedState" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public abstract void setExpandedState(TreePath path, boolean isExpanded)
~~~

## Parámetros
* **TreePath path**,  {% include w3api/param_description.html metodo=_dato parametro="TreePath path" %}
* **boolean isExpanded**,  {% include w3api/param_description.html metodo=_dato parametro="boolean isExpanded" %}

## Clase Padre
[AbstractLayoutCache](/Java/AbstractLayoutCache/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in _dato.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
