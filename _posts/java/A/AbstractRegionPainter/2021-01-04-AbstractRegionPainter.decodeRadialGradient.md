---
title: AbstractRegionPainter.decodeRadialGradient()
permalink: Java/AbstractRegionPainter/decodeRadialGradient
date: 2021-01-04
key: JavaJava.A.AbstractRegionPainter
category: java
tags: ['java se', 'javax.swing.plaf.nimbus', 'java.desktop', 'metodo java', 'Java 1.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.A.AbstractRegionPainter.metodos valor="decodeRadialGradient" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
protected RadialGradientPaint decodeRadialGradient(float x, float y, float r, float[] midpoints, Color[] colors)
~~~

## Parámetros
* **float x**,  {% include w3api/param_description.html metodo=_data parametro="float x" %}
* **float y**,  {% include w3api/param_description.html metodo=_data parametro="float y" %}
* **float r**,  {% include w3api/param_description.html metodo=_data parametro="float r" %}
* **float[] midpoints**,  {% include w3api/param_description.html metodo=_data parametro="float[] midpoints" %}
* **Color[] colors**,  {% include w3api/param_description.html metodo=_data parametro="Color[] colors" %}

## Clase Padre
[AbstractRegionPainter](/Java/AbstractRegionPainter/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.A.AbstractRegionPainter.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
