---
title: AbstractRegionPainter.decodeAnchorX()
permalink: Java/AbstractRegionPainter/decodeAnchorX
date: 2021-01-11
key: JavaJava.A.AbstractRegionPainter
category: java
tags: ['java se', 'javax.swing.plaf.nimbus', 'java.desktop', 'metodo java', 'Java 1.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.A.AbstractRegionPainter.metodos valor="decodeAnchorX" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
protected float decodeAnchorX(float x, float dx)
~~~

## Parámetros
* **float x**,  {% include w3api/param_description.html metodo=_dato parametro="float x" %}
* **float dx**,  {% include w3api/param_description.html metodo=_dato parametro="float dx" %}

## Clase Padre
[AbstractRegionPainter](/Java/AbstractRegionPainter/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in _dato.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
