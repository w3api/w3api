---
title: ORB.create_sequence_tc()
permalink: Java/ORB-org-omg-CORBA/create_sequence_tc
date: 2021-01-04
key: JavaJava.O.ORB-org-omg-CORBA
category: java
tags: ['java se', 'org.omg.CORBA', 'java.corba', 'metodo java', 'JDKJava 1.2']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.O.ORB-org-omg-CORBA.metodos valor="create_sequence_tc" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public abstract TypeCode create_sequence_tc(int bound, TypeCode element_type)
~~~

## Parámetros
* **int bound**,  {% include w3api/param_description.html metodo=_data parametro="int bound" %}
* **TypeCode element_type**,  {% include w3api/param_description.html metodo=_data parametro="TypeCode element_type" %}

## Clase Padre
[ORB](/Java/ORB-org-omg-CORBA/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.O.ORB-org-omg-CORBA.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
