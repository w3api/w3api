---
title: ObjectAlreadyActiveHelper.write()
permalink: Java/ObjectAlreadyActiveHelper/write
date: 2021-01-04
key: JavaJava.O.ObjectAlreadyActiveHelper
category: java
tags: ['java se', 'org.omg.PortableServer.POAPackage', 'java.corba', 'metodo java', 'Java 1.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.O.ObjectAlreadyActiveHelper.metodos valor="write" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public static void write(OutputStream ostream, ObjectAlreadyActive value)
~~~

## Parámetros
* **ObjectAlreadyActive value**,  {% include w3api/param_description.html metodo=_data parametro="ObjectAlreadyActive value" %}
* **OutputStream ostream**,  {% include w3api/param_description.html metodo=_data parametro="OutputStream ostream" %}

## Clase Padre
[ObjectAlreadyActiveHelper](/Java/ObjectAlreadyActiveHelper/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.O.ObjectAlreadyActiveHelper.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
