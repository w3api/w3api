---
title: ResourceContextFactory.getResourceRequest()
permalink: /Java/ResourceContextFactory/getResourceRequest/
date: 2021-01-11
key: Java.R.ResourceContextFactory
category: Java
tags: ['java se', 'jdk.management.resource', 'jdk.management.resource', 'metodo java', '8u40']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.R.ResourceContextFactory.metodos valor="getResourceRequest" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public ResourceRequest getResourceRequest(ResourceType type)
~~~

## Parámetros
* **ResourceType type**,  {% include w3api/param_description.html metodo=_dato parametro="ResourceType type" %}

## Clase Padre
[ResourceContextFactory](/Java/ResourceContextFactory/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in _dato.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
