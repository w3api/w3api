---
title: MixerProvider.getMixer()
permalink: Java/MixerProvider/getMixer
date: 2021-01-04
key: JavaJava.M.MixerProvider
category: java
tags: ['java se', 'javax.sound.sampled.spi', 'java.desktop', 'metodo java', 'Java 1.3']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.M.MixerProvider.metodos valor="getMixer" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public abstract Mixer getMixer(Mixer.Info info)
~~~

## Parámetros
* **Mixer.Info info**,  {% include w3api/param_description.html metodo=_data parametro="Mixer.Info info" %}

## Excepciones
[IllegalArgumentException](/Java/IllegalArgumentException/)

## Clase Padre
[MixerProvider](/Java/MixerProvider/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.M.MixerProvider.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
