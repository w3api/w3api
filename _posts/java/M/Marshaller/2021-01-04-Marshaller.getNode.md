---
title: Marshaller.getNode()
permalink: Java/Marshaller/getNode
date: 2021-01-04
key: JavaJava.M.Marshaller
category: java
tags: ['java se', 'javax.xml.bind', 'java.xml.bind', 'metodo java', 'Java 1.6', 'JAXB Java 1.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.M.Marshaller.metodos valor="getNode" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
Node getNode(Object contentTree) throws JAXBException
~~~

## Parámetros
* **Object contentTree**,  {% include w3api/param_description.html metodo=_data parametro="Object contentTree" %}

## Excepciones
[UnsupportedOperationException](/Java/UnsupportedOperationException/), [JAXBException](/Java/JAXBException/), [IllegalArgumentException](/Java/IllegalArgumentException/)

## Clase Padre
[Marshaller](/Java/Marshaller/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in site.data.Java.M.Marshaller.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
