---
title: MetalTabbedPaneUI.paintRightTabBorder()
permalink: Java/MetalTabbedPaneUI/paintRightTabBorder
date: 2021-01-11
key: JavaJava.M.MetalTabbedPaneUI
category: Java
tags: ['java se', 'javax.swing.plaf.metal', 'java.desktop', 'metodo java', 'Java 1.0']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.M.MetalTabbedPaneUI.metodos valor="paintRightTabBorder" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
protected void paintRightTabBorder(int tabIndex, Graphics g, int x, int y, int w, int h, int btm, int rght, boolean isSelected)
~~~

## Parámetros
* **Graphics g**,  {% include w3api/param_description.html metodo=_dato parametro="Graphics g" %}
* **int h**,  {% include w3api/param_description.html metodo=_dato parametro="int h" %}
* **int y**,  {% include w3api/param_description.html metodo=_dato parametro="int y" %}
* **int btm**,  {% include w3api/param_description.html metodo=_dato parametro="int btm" %}
* **int w**,  {% include w3api/param_description.html metodo=_dato parametro="int w" %}
* **int rght**,  {% include w3api/param_description.html metodo=_dato parametro="int rght" %}
* **int x**,  {% include w3api/param_description.html metodo=_dato parametro="int x" %}
* **boolean isSelected**,  {% include w3api/param_description.html metodo=_dato parametro="boolean isSelected" %}
* **int tabIndex**,  {% include w3api/param_description.html metodo=_dato parametro="int tabIndex" %}

## Clase Padre
[MetalTabbedPaneUI](/Java/MetalTabbedPaneUI/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Líneas de Código
<ul>
{%- for _ldc in _dato.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
