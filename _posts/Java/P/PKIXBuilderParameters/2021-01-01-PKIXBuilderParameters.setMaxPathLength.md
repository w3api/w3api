---
title: PKIXBuilderParameters.setMaxPathLength()
permalink: /Java/PKIXBuilderParameters/setMaxPathLength/
date: 2021-01-11
key: Java.P.PKIXBuilderParameters
category: Java
tags: ['java se', 'java.security.cert', 'java.base', 'metodo java', 'Java 1.4']
sidebar: 
  nav: java
---

{% include w3api/datos.html clase=site.data.Java.P.PKIXBuilderParameters.metodos valor="setMaxPathLength" %}

## Descripción
{{_dato.description }}

## Sintaxis
~~~java
public void setMaxPathLength(int maxPathLength)
~~~

## Parámetros
* **int maxPathLength**,  {% include w3api/param_description.html metodo=_dato parametro="int maxPathLength" %}

## Excepciones
[InvalidParameterException](/Java/InvalidParameterException/)

## Clase Padre
[PKIXBuilderParameters](/Java/PKIXBuilderParameters/)

## Ejemplo
~~~java
{{ _dato.code}}
~~~

## Artículos
<ul>
{%- for _ldc in _dato.ldc -%}
   <li>
       <a href="{{_ldc['url'] }}">{{ _ldc['nombre'] }}</a>
   </li>
{%- endfor -%}
</ul>
